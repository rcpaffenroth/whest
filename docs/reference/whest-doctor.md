...
------------
  ...
  [OK]    whest install mode     from-source · https://github.com/AIcrowd/whestbench.git
  [OK]    Node.js on PATH        v16.20.2
  ...
  [OK]    Free disk in CWD       169.5 GiB free
  [OK]    CWD writable           /Users/mohanty/work/AIcrowd/challenges/alignment-research-center/whest-starterkit/.claude/worktrees/unruffled-newton-8a6c58

  7 checks · 6 ok · 1 warn · 0 fail
```
...
| **whest install mode** | Reports whether `whestbench` was installed from a published wheel or `git+https://…@<sha>` (the starter-kit pin). | None — informational. If you see `from-source`, your runs match the kit's pinned upstream commit. |
| **Node.js on PATH** | Required for `whest visualizer` (the explorer browser UI). | Install Node 16+ if you plan to use the visualizer. Otherwise the warning is harmless. |
