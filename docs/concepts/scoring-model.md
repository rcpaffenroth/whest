...
The table below shows the **raw-MSE diagnostics** from the bundled example estimators, via `uv run whest run --estimator examples/<NN>_<name>.py --n-mlps 10 --seed 42` at the default settings (width=256, depth=8, 10 MLPs, 6.8e10 FLOP budget). These are the unscaled `final_layer_mse` / `all_layers_mse` values; the leaderboard `adjusted_final_layer_score` multiplies `final_layer_mse` by `max(0.1, C_m / budget)` (≤ 1.0). Every bundled example spends well under 1% of the budget, so each one bottoms out at the **0.1 floor** — its ranked score is exactly `final_layer_mse ÷ 10`. Use these as calibration points for your own estimator.

| Estimator | `final_layer_mse` | `all_layers_mse` | Approach |
|-----------|-----------|---------------|----------|
| `random_estimator` | ~0.56 | ~0.42 | Returns random values — the interface walkthrough. The bundled [`estimator.py`](../../estimator.py) at the repo root is the true (all-zeros) baseline (~0.77); running `uv run whest init <dir>` in a fresh directory produces the same template. |
| `mean_propagation` | ~7.7e-04 | ~4.3e-04 | Diagonal variance, O(depth x width^2), ~270M FLOPs. ~1000x better than the zeros baseline. |
| `covariance_propagation` | ~3.7e-05 | ~1.8e-05 | Full covariance, O(depth x width^3), ~540M FLOPs. ~20x better again than mean propagation. |

**How to read these numbers:**

- The **zeros baseline** (`estimator.py`, ~0.77) and the **random estimator** (~0.56) give you the "doing nothing" scale — their MSE reflects the natural magnitude of the ground-truth activations.
- **Mean propagation** is ~1000x more accurate than zeros — a huge improvement from a simple analytical formula at ~270M FLOPs (well under 1% of budget).
- **Covariance propagation** is another ~20x better, but costs O(width^3) per layer (~540M FLOPs at width=256, still <1% of budget). The cubic cost grows fast — by around width≈1300 it would consume the whole 6.8e10 budget.
